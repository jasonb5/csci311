...
void Playlist::processCommand() {
   string s;
   getline(cin, s);                         // get everything to std::endl
 ...
 *
 * Plays (prints to std::cout) the current song, if there is one.
 */
 ...
 * the last song, each song is played (printed to std::cout). When finished, the
 ...
 * reverse order to the first song, each song is played (printed to std::cout).
